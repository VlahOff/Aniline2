...
import { cryptoActions } from '../slices/cryptoSlice';

export const getTopThree = () => {
	return dispatch => {
		// dispatch(uiActions.startLoading());
		cryptoService.fetchTopThree().then(res => {
			// if (res.errorMessage) {
			// 	dispatch(
...
export const getNextPageOnTopHundred = page => {
	return dispatch => {
		// dispatch(uiActions.startLoading());
		cryptoService.fetchTopHundred(page).then(res => {
			// if (res.errorMessage) {
			// 	dispatch(
			// 		uiActions.setNotificationMessage({
			// 			message: res.errorMessage,
...
export const getNextPageOnNewCoins = page => {
	return dispatch => {
		// dispatch(uiActions.startLoading());
		cryptoService.fetchNewCoins(page).then(res => {
			// if (res.errorMessage) {
			// 	dispatch(
			// 		uiActions.setNotificationMessage({
			// 			message: res.errorMessage,
...
export const getCoinDetails = (id, days) => {
	return dispatch => {
		...
		Promise.all([
			...
			dispatch(cryptoActions.setCoinDetailsOHLC(res[1]));
		});
		// .catch(err =>
		// 	dispatch(
		// 		uiActions.setNotificationMessage({
...
export const getSearchResults = query => {
	return dispatch => {
		// dispatch(uiActions.startLoading());
		cryptoService.fetchSearchResults(query).then(res => {
			// if (res.errorMessage) {
			// 	dispatch(
			// 		uiActions.setNotificationMessage({
			// 			message: res.errorMessage,
